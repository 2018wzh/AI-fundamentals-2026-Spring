...
## 6. `textual/OilETF/OilETF_report.csv`
...
## 7. `textual/OilETF/OilETF_search.csv`
...
```text
images/OilETF/{symbol}_{end_date}_H{H}.png
```

示例：

```text
images/OilETF/USO_2024-01-10_H60.png
```
...
```text
1. numerical/OilETF/OilETF.csv
2. textual/OilETF/OilETF_search.csv
...
(ROOT / "numerical/OilETF").mkdir(parents=True, exist_ok=True)
(ROOT / "textual/OilETF").mkdir(parents=True, exist_ok=True)
...
num.to_csv(ROOT / "numerical/OilETF/OilETF.csv", index=False)
```
...
text.to_csv(ROOT / "textual/OilETF/OilETF_search.csv", index=False)
```
...
| 交付物 | 文件 |
|---|---|
| 数据说明 | `metadata/OilETF_dataset_card.json` |
| Time-MMD 数值数据 | `numerical/OilETF/OilETF.csv` |
| Time-MMD 文本数据 | `textual/OilETF/OilETF_search.csv`, `OilETF_report.csv` |
