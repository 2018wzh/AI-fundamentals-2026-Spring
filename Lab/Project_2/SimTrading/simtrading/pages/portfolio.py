...
def render_portfolio_page(dataset_bundle: DatasetBundle, strategy_config: StrategyConfig) -> None:
    ...
    st.plotly_chart(make_equity_figure(result.equity_curve), width="stretch")
    st.plotly_chart(make_drawdown_figure(result.equity_curve), width="stretch")
    st.plotly_chart(make_monthly_return_heatmap(result.equity_curve), width="stretch")
    st.dataframe(result.equity_curve, width="stretch")
