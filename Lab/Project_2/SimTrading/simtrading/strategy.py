...
def get_strategy_availability(predictions: pd.DataFrame) -> dict[StrategyName, tuple[bool, str]]:
    ...
    if not QUANTILE_COLUMNS.issubset(predictions.columns):
        ...
        invalid_intervals = (predictions["q90"] - predictions["q10"]) <= 0
        if invalid_intervals.any():
            availability[StrategyName.CONFIDENCE_LONG_CASH] = (
                False,
                "Unavailable because at least one prediction row has a non-positive q10-q90 interval.",
            )
    symbol_counts = predictions.groupby("end_date")["symbol"].nunique()
    if symbol_counts.empty or int(symbol_counts.max()) < 2:
        availability[StrategyName.TOP_K_ROTATION] = (
            False,
            "Unavailable because no prediction date has multiple unique symbols.",
        )
    return availability
...
def validate_strategy_inputs(predictions: pd.DataFrame, strategy_config: StrategyConfig) -> None:
    ...
    if strategy_config.name == StrategyName.TOP_K_ROTATION:
        ...
        if int(counts.max()) < strategy_config.top_k:
            raise StrategyError("`Top-K Rotation` requires at least one trading date with `top_k` unique symbols.")
...
def build_signal_frame(predictions: pd.DataFrame, strategy_config: StrategyConfig) -> pd.DataFrame:
    ...
    if strategy_config.name == StrategyName.LONG_CASH:
        ...
        signals["score"] = signals[strategy_config.score_column]
        counts = signals.groupby("end_date")["symbol"].transform("nunique")
        signals = signals.loc[counts >= strategy_config.top_k].copy()
        if signals.empty:
            raise StrategyError("`Top-K Rotation` has no dates with enough symbols after filtering.")
