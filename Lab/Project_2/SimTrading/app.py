...
def _render_sidebar(config_path: Path):
    ...
    model_options = [
        model
        for model in dataset_cfg.prediction_result_paths
        if prediction_file_supports_setting(dataset_cfg, model, setting_key)
    ]
    if not model_options:
        st.error(f"No configured prediction file has rows for `{dataset_cfg.dataset_label}` / `{setting_key}`.")
        st.stop()
